...
public class RindRaidSpawn {
	private static Logger _log = Logger.getLogger(RindRaidSpawn.class
			.getName());

	private static RindRaidSpawn _instance;

	public static RindRaidSpawn getInstance() {
		if (_instance == null) {
			_instance = new RindRaidSpawn();
		}
		return _instance;
	}

	private RindRaidSpawn() {
	...
	public void fillSpawnTable(int mapid, int type) {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM spawnlist_rind");
