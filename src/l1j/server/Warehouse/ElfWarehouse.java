...
public class ElfWarehouse extends Warehouse {
	...
	public void loadItems() {
		...
		try {
			...
			_log.log(Level.SEVERE, "ElfWarehouse[]Error", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	public void insertItem(L1ItemInstance item) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("INSERT INTO character_elf_warehouse SET id = ?, account_name = ?, item_id = ?, item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, attr_enchantlvl = ?, bless = ?, , second_id=?, round_id=?, ticket_id=?, package=?");
			...
			_log.log(Level.SEVERE, "ElfWarehouse[]Error1", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public void updateItem(L1ItemInstance item) {
		...
		try {
			...
			_log.log(Level.SEVERE, "ElfWarehouse[]Error2", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public void deleteItem(L1ItemInstance item) {
		...
		try {
			...
			_log.log(Level.SEVERE, "ElfWarehouse[]Error3", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
