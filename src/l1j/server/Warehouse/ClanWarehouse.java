...
public class ClanWarehouse extends Warehouse {
	...
	public synchronized void loadItems() {
		...
		try {
			...
			_log.log(Level.SEVERE, "ClanWarehouse[]Error", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	public synchronized void insertItem(L1ItemInstance item) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("INSERT INTO clan_warehouse SET id = ?, clan_name = ?, item_id = ?, item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id= ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, attr_enchantlvl = ?, package = ?");
			...
			_log.log(Level.SEVERE, "ClanWarehouse[]Error1", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public synchronized void updateItem(L1ItemInstance item) {
		...
		try {
			...
			_log.log(Level.SEVERE, "ClanWarehouse[]Error2", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public synchronized void deleteItem(L1ItemInstance item) {
		...
		try {
			...
			_log.log(Level.SEVERE, "ClanWarehouse[]Error3", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public synchronized void deleteAllItems() {
		...
		try {
			...
			_log.log(Level.SEVERE, "ClanWarehouse[]Error4", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
