...
public class PrivateWarehouse extends Warehouse {
	...
	protected static Logger _log = Logger.getLogger(PrivateWarehouse.class
			.getName());
	...
	public synchronized void loadItems() {
		...
		try {
			...
			_log.log(Level.SEVERE, "PrivateWarehouse[]Error", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	public synchronized void insertItem(L1ItemInstance item) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("INSERT INTO character_warehouse SET id = ?, account_name = ?, item_id = ?, item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, attr_enchantlvl = ?, bless = ?, second_id=?, round_id=?, ticket_id=?, package=?");
			...
			pstm.setInt(17, item.isPackage() == false ? 0 : 1);
			pstm.execute();
		} catch (SQLException e) {
			_log.log(Level.SEVERE, "PrivateWarehouse[]Error1", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public synchronized void updateItem(L1ItemInstance item) {
		...
		try {
			...
			_log.log(Level.SEVERE, "PrivateWarehouse[]Error2", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public synchronized void deleteItem(L1ItemInstance item) {
		...
		try {
			...
			_log.log(Level.SEVERE, "PrivateWarehouse[]Error3", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
