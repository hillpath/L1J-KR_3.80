...
public class S_SPMR extends ServerBasePacket {
	...
	private void buildPacket(L1PcInstance pc) {
		...
		if (pc.getSkillEffectTimerSet().hasSkillEffect(
			...
			writeC(pc.getAbility().getSp() - pc.getAbility().getTrueSp() - 2); // 장비
			// 증가한
			// SP
		} else {
			writeC(pc.getAbility().getSp() - pc.getAbility().getTrueSp()); // 장비
			// 증가한
			// SP
		}
		writeC(pc.getResistance().getMr() - pc.getResistance().getBaseMr()); // 장비나
		// 마법으로
		// 증가한
		// MR
	}
