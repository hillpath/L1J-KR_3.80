...
public class L1Astar {
	...
	public L1Node FindPath(L1Object npc, int tx, int ty, int mapId, L1Character target) {
		...
		while (count < LIMIT_LOOP) {
			...
			if( MakeChild(best, tx, ty, npc.getMapId(), target) == 0 && count == 0 ) {
				return null;
			}
			count++;
		}
	...
	// *************************************************************************
	public char MakeChild(L1Node node, int tx, int ty, short m, L1Character target) {
		int		x, y;
		char	flag = 0;
		...
		y = node.y;
		// 인접한 노드로 이동가능한지 검사
		cc[0] = IsMove(x, y + 1, m, target);
		cc[1] = IsMove(x - 1, y + 1, m, target);
		cc[2] = IsMove(x - 1, y, m, target);
		cc[3] = IsMove(x - 1, y - 1, m, target);
		cc[4] = IsMove(x, y - 1, m, target);
		cc[5] = IsMove(x + 1, y - 1, m, target);
		cc[6] = IsMove(x + 1, y, m, target);
		cc[7] = IsMove(x + 1, y + 1, m, target);
	...
	// Desc : 이동가능한 위치인지 검사
	// *************************************************************************
	public char IsMove(int x, int y, short mapid, L1Character target) {
		...
		*/
		...
		for (L1Object obj : L1World.getInstance().getVisibleObjects(target, 1)) {
