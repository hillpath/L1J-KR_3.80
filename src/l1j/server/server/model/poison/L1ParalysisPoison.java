...
public class L1ParalysisPoison extends L1Poison {
	...
	private class ParalysisPoisonTimer extends Thread {
		@Override
		public void run() {
			_target.getSkillEffectTimerSet().setSkillEffect(L1SkillId.STATUS_POISON_PARALYZING, 0);

			try {
				Thread.sleep(_delay); 
			} catch (InterruptedException e) {
				_target.getSkillEffectTimerSet().killSkillEffectTimer(L1SkillId.STATUS_POISON_PARALYZING);
				return;
			}
	...
	private class ParalysisTimer extends Thread {
		...
		public void run() {
			_target.getSkillEffectTimerSet().killSkillEffectTimer(L1SkillId.STATUS_POISON_PARALYZING);
			_target.getSkillEffectTimerSet().setSkillEffect(L1SkillId.STATUS_POISON_PARALYZED, 0);
			try {
				Thread.sleep(_time);
			} catch (InterruptedException e) {
			}

			_target.getSkillEffectTimerSet().killSkillEffectTimer(L1SkillId.STATUS_POISON_PARALYZED);
			if (_target instanceof L1PcInstance) {
				L1PcInstance player = (L1PcInstance) _target;
				if (!player.isDead()) {
