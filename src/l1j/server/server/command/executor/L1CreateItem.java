	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			CodeLogger.getInstance().gmlog(
					"GMCOMMAND",
					"캐릭=" + pc.getName() + "[" + pc.getAccountName() + "]" + "명령어=" + "  아이템   " + arg + "	IP="
				...
				int enchant = 0;
				if (st.hasMoreTokens()) { enchant = Integer.parseInt(st.nextToken());
				}
				int attrenchant = 0;
				if (st.hasMoreTokens()) { attrenchant = Integer.parseInt(st.nextToken());
				}
				int isId = 0;
				if (st.hasMoreTokens()) {
				...
				} catch (NumberFormatException e) {	itemid = ItemTable.getInstance().findItemIdByNameWithoutSpace(nameid);
					if (itemid == 0) {
			...
			pc.sendPackets(new S_SystemMessage(".아이템(이름/ID) [갯수] [인챈트수] [속성] [확인] 라고 입력해 주세요. "));
		}
	}
}
