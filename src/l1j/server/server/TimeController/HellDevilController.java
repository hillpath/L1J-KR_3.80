...
public class HellDevilController extends Thread {

	private static HellDevilController _instance;
	...
	public static HellDevilController getInstance() {
		if (_instance == null) {
			_instance = new HellDevilController();
		}
		return _instance;
	}
	...
	public void run() {
		try {
			while (true) {
				...
				/** 오픈 메세지 * */
				L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE,"\\f3지옥 사냥터가 열렸습니다."));
				/** 악마왕영토 시작* */
				setDevilStart(true);
	...
	private void TelePort() {
		for (L1PcInstance c : L1World.getInstance().getAllPlayers()) {
			...
			case 666:
				...
				c.sendPackets(new S_SystemMessage("지옥사냥터가 닫혔습니다."));
				break;
			default:
				break;
			}
	...
	/** 종료 * */
	private void End() {
		L1World.getInstance().broadcastServerMessage("지옥사냥터가 사라졌습니다.3시간마다 열립니다.");
		setDevilStart(false);
	}
}
