...
public class HomeTownController {
	private static Logger _log = Logger.getLogger(HomeTownController.class
			.getName());
	...
	public void monthlyProc() {
		...
		for (L1PcInstance pc : list) {
			try {
				...
				_log.log(Level.SEVERE, "HomeTownController[]Error", e);
			}
		}
		...
		for (L1PcInstance pc : list) {
			...
			try {
				...
				_log.log(Level.SEVERE, "HomeTownController[]Error1", e);
			}
		}
		clearHomeTownID();
	...
	private static String totalContribution(int townId) {
		...
		try {
			...
			_log.log(Level.SEVERE, "HomeTownController[]Error2", e);
		} finally {
			SQLUtil.close(rs1);
			SQLUtil.close(rs2);
			SQLUtil.close(rs3);
	...
	private static void clearHomeTownID() {
		...
		try {
			...
			_log.log(Level.SEVERE, "HomeTownController[]Error3", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public static int getPay(int objid) {
		...
		try {
			...
			_log.log(Level.SEVERE, "HomeTownController[]Error4", e);
		} finally {
			SQLUtil.close(rs1);
			SQLUtil.close(pstm1);
			SQLUtil.close(pstm2);
