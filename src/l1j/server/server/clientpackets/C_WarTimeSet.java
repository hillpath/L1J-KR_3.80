...
public class C_WarTimeSet extends ClientBasePacket {
	...
	public C_WarTimeSet(byte abyte0[], LineageClient clientthread)
			throws Exception {
		super(abyte0);
		/*
		 * int listNo = readC();
		 ...
		 * warTime.set(year, month, date, 18, 30); msg = ""+year+"년
		 * "+(month+1)+"월 "+date+"일 18시 30분"; break; case 3: warTime.set(year,
		 * month, date, 19, 00); msg = ""+year+"년 "+(month+1)+"월 "+date+"일 19시
		 * 00분"; break; case 4: warTime.set(year, month, date, 22, 00); msg =
		 * ""+year+"년 "+(month+1)+"월 "+date+"일 22시 00분"; break; case 5:
		 * warTime.set(year, month, date, 22, 30); msg = ""+year+"년
		 * "+(month+1)+"월 "+date+"일 22시 30분"; break; case 6: warTime.set(year,
		 * month, date, 23, 00); msg = ""+year+"년 "+(month+1)+"월 "+date+"일 23시
		 * 00분"; break; default : break; }
		 * CastleTable.getInstance().updateCastle(l1castle); pc.sendPackets(new
		 * S_ServerMessage(304, msg));// 다음 공성전 시간이 %0로 결정되었습니다. } }
		 */
	}
