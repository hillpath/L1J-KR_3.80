...
public class C_War extends ClientBasePacket {
	...
	public C_War(byte abyte0[], LineageClient clientthread) throws Exception {
		...
		if (!player.isCrown()) { // 군주 이외
			player.sendPackets(new S_ServerMessage(478));// \f1프린스와 프린세스만 전쟁을
			// 포고할 수 있습니다.
			return;
		}
		if (clanId == 0) { // 크란미소속
			player.sendPackets(new S_ServerMessage(272)); // \f1전쟁하기 위해서는 우선
			// 혈맹을 창설하지 않으면
			// 안됩니다.
			return;
		}
		...
		if (clan.getAlliance() == enemyClan.getClanId()) {
			player.sendPackets(new S_ServerMessage(1205)); // 동맹혈맹과는 전쟁을 할 수
			// 없습니다.
			return;
		}
		...
		for (L1War war : warList) {
			if (war.CheckClanInWar(clanName)) { // 자크란이 이미 전쟁중
				if (type == 0) { // 선전포고
					player.sendPackets(new S_ServerMessage(234)); // \f1당신의
					// 혈맹은 벌써
					// 전쟁중입니다.
					return;
				}
		...
		if (clan.getCastleId() != 0) { // 자크란이 성주
			if (type == 0) { // 선전포고
				player.sendPackets(new S_ServerMessage(474)); // 당신은 벌써 성을
				// 소유하고 있으므로, 다른
				// 시로를 잡을 수
				// 없습니다.
				return;
		...
		if (enemyClan.getCastleId() == 0 && // 상대 크란이 성주는 아니고, 자캐릭터가 Lv15 이하
				player.getLevel() <= 15) {
			player.sendPackets(new S_ServerMessage(232)); // \f1레벨 15 이하의 군주는
			// 선전포고할 수 없습니다.
			return;
		}
		...
		if (enemyClan.getCastleId() != 0 && // 상대 크란이 성주로, 자캐릭터가 Lv25 미만
				player.getLevel() < 50) {
			// player.sendPackets(new S_ServerMessage(475)); // 공성전을 선언하려면 레벨
			// 25에 이르지 않으면 안됩니다.
			player.sendPackets(new S_SystemMessage(
					"공성전을 선언하려면  레벨 50에 이르지 않으면 안됩니다.")); // 공성전을
			// 선언하려면
			// 레벨
			// 25에
			// 이르지
		...
		/** 2011.08.26 혈마크 없이 공성 포고 불가 */

		File file = new File(System.getProperty("user.dir") + "/emblem/"
				+ clan.getClanId());

		if (!file.exists()) {
			player.sendPackets(new S_SystemMessage("혈마크 없이는 전쟁을 포고할 수 없습니다."));
			return;
		}
		if (clan.getOnlineClanMember().length <= 5) {
			player.sendPackets(new S_SystemMessage(
				"현재 접속한 혈맹 구성원이 [5]명 이상 되어야 공성전 선포가 가능합니다."));
			return;
		}

		// if (clan.getHouseId() > 0){
		// player.sendPackets(new S_SystemMessage("아지트가 있는 상태에서는 선전 포고를 할 수
		// 없습니다."));
		// return;
		// }
		if (enemyClan.getCastleId() != 0) { // 상대 크란이 성주
			...
			if (WarTimeController.getInstance().isNowWar(castle_id)) { // 전쟁
				...
				for (L1War war : warList) {
					if (war.CheckClanInWar(enemyClanName)) { // 상대 크란이 이미 전쟁중
						if (type == 0) { // 선전포고
							...
							if (!war
									.CheckClanInSameWar(clanName, enemyClanName)) { // 자크란과
								// 상대
								// 크란이
								// 다른
								// 전쟁
				...
				if (type == 0) { // 선전포고
					player.sendPackets(new S_ServerMessage(476)); // 아직 공성전의
					// 시간이 아닙니다.
				}
			}
		} else { // 상대 크란이 성주는 아니다
			...
			if (!enemyInWar && (type == 2 || type == 3)) { // 상대 크란이 전쟁중 이외로,
				// 항복 또는 종결
				return;
			}
			...
			if (type == 0) { // 선전포고
				...
				enemyLeader.sendPackets(new S_Message_YN(217, clanName,
						playerName)); // %0혈맹의%1가 당신의 혈맹과의 전쟁을 바라고 있습니다. 전쟁에
				// 응합니까? (Y/N)
			} else if (type == 2) { // 항복
				enemyLeader.setTempID(player.getId()); // 상대의 오브젝트 ID를 보존해 둔다
				enemyLeader.sendPackets(new S_Message_YN(221, clanName)); // %0혈맹이
				// 항복을
				// 바라고
				// 있습니다.
				// 받아들입니까?
				...
				enemyLeader.setTempID(player.getId()); // 상대의 오브젝트 ID를 보존해 둔다
				enemyLeader.sendPackets(new S_Message_YN(222, clanName)); // %0혈맹이
				// 전쟁의
				// 종결을
				// 바라고
				// 있습니다.
