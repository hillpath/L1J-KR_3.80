...
public class NpcCashShopTable {
	...
	private static Logger _log = Logger.getLogger(NpcCashShopTable.class.getName());
	...
	private ArrayList<Integer> enumNpcIds() {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT DISTINCT npc_id FROM cash_shop_npc");
	...
	private void loadShops() {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM cash_shop_npc WHERE npc_id=?");
			L1Shop shop = null;
