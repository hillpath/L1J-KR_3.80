...
public class NpcTable {
	...
	private void loadNpcData() {
		...
		try {
			...
			_log.log(Level.SEVERE, "NpcTable[]Error", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	public static Map<String, Integer> buildFamily() {
		...
		try {
			...
			_log.log(Level.SEVERE, "NpcTable[]Error1", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	public void ReloadNpcData(String name) {
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		try {
			...
			_log.log(Level.SEVERE, "NpcTable[]Error2", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	public void ReloadNewNpcData() {
		...
		try {
			...
			_log.log(Level.SEVERE, "NpcTable[]Error3", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
