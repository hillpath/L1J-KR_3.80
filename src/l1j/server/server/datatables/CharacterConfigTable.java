...
public class CharacterConfigTable {
	...
	public void storeCharacterConfig(int objectId, int length, byte[] data) {
		...
		try {
			...
			_log.log(Level.SEVERE, "CharacterConfigTable[]Error", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public void updateCharacterConfig(int objectId, int length, byte[] data) {
		...
		try {
			...
			_log.log(Level.SEVERE, "CharacterConfigTable[]Error1", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public void deleteCharacterConfig(int objectId) {
		...
		try {
			...
			_log.log(Level.SEVERE, "CharacterConfigTable[]Error2", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public int countCharacterConfig(int objectId) {
		...
		try {
			...
			_log.log(Level.SEVERE, "CharacterConfigTable[]Error3", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
