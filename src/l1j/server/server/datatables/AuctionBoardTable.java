...
import java.text.SimpleDateFormat; // ########## (넬) A18 AuctionBoardTable 오류
// 수정 위해 임포트 추가 ##########
...
public class AuctionBoardTable {
	...
	public AuctionBoardTable() {
		...
		try {
			...
			_log.log(Level.SEVERE, "AuctionBoardTable[]Error", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	public void insertAuctionBoard(L1AuctionBoard board) {
		...
		try {
			...
			_log.log(Level.SEVERE, "AuctionBoardTable[]Error1", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public void updateAuctionBoard(L1AuctionBoard board) {
		...
		try {
			...
			_log.log(Level.SEVERE, "AuctionBoardTable[]Error2", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public void deleteAuctionBoard(int houseId) {
		...
		try {
			...
			_log.log(Level.SEVERE, "AuctionBoardTable[]Error3", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
