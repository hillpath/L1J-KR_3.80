...
public class FurnitureSpawnTable {
	...
	private void FillFurnitureSpawnTable() {
		...
		try {
			...
			_log.log(Level.SEVERE, "FurnitureSpawnTable[]Error", e);
		} catch (SecurityException e) {
			_log.log(Level.SEVERE, "FurnitureSpawnTable[]Error1", e);
		} catch (ClassNotFoundException e) {
			_log.log(Level.SEVERE, "FurnitureSpawnTable[]Error2", e);
		} catch (IllegalArgumentException e) {
			_log.log(Level.SEVERE, "FurnitureSpawnTable[]Error3", e);
		} catch (InstantiationException e) {
			_log.log(Level.SEVERE, "FurnitureSpawnTable[]Error4", e);
		} catch (IllegalAccessException e) {
			_log.log(Level.SEVERE, "FurnitureSpawnTable[]Error5", e);
		} catch (InvocationTargetException e) {
			_log.log(Level.SEVERE, "FurnitureSpawnTable[]Error6", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	public void insertFurniture(L1FurnitureInstance furniture) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("INSERT INTO spawnlist_furniture SET item_obj_id=?, npcid=?, locx=?, locy=?, mapid=?, house_Id=?");
			...
			_log.log(Level.SEVERE, "FurnitureSpawnTable[]Error7", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public void deleteFurniture(L1FurnitureInstance furniture) {
		...
		try {
			...
			_log.log(Level.SEVERE, "FurnitureSpawnTable[]Error8", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public void deleteAllFurniture(int HouseId) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("DELETE FROM spawnlist_furniture WHERE house_Id=?");
			pstm.setInt(1, HouseId);
			pstm.execute();
		} catch (SQLException e) {
			_log.log(Level.SEVERE, "FurnitureSpawnTable[]Error9", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
