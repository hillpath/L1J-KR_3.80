...
public class DoorSpawnTable {
		...
		try {
			...
			do {
				...
				if (l1npc != null) {
					...
					door.getGfxId().setGfxId(rs.getInt(3));
			...
			_log.log(Level.SEVERE, "DoorSpawnTable[]Error", e);
		} catch (SecurityException e) {
			_log.log(Level.SEVERE, "DoorSpawnTable[]Error1", e);
		} catch (ClassNotFoundException e) {
			_log.log(Level.SEVERE, "DoorSpawnTable[]Error2", e);
		} catch (IllegalArgumentException e) {
			_log.log(Level.SEVERE, "DoorSpawnTable[]Error3", e);
		} catch (InstantiationException e) {
			_log.log(Level.SEVERE, "DoorSpawnTable[]Error4", e);
		} catch (IllegalAccessException e) {
			_log.log(Level.SEVERE, "DoorSpawnTable[]Error5", e);
		} catch (InvocationTargetException e) {
			_log.log(Level.SEVERE, "DoorSpawnTable[]Error6", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	public L1DoorInstance getDoor(int npcId) {
		...
		for (L1DoorInstance door : _doorList
				.toArray(new L1DoorInstance[_doorList.size()])) {
