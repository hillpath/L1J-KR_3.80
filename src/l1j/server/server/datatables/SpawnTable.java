	...
	private void fillSpawnTable() {
		...
		try {
			...
			while (rs.next()) {
				...
				if (Config.ALT_HALLOWEENEVENT == false && spawnid == 1)
					continue;
				if (Config.ALT_CHUSEOKEVENT == false && spawnid == 2)
					continue;
				if (Config.ALT_HALLOWEENEVENT2009 == false && spawnid == 3)
					continue;
	...
	public void removeSpawn(L1NpcInstance paramL1NpcInstance)
	  {
	    ...
	    try
	    {
	      ...
	      this._spawntable.remove(Integer.valueOf(i));
	      	pstm = con.prepareStatement("delete from spawnlist where npc_templateid=? and mapid=? and locx=? and locy=?");
	      	pstm.setInt(1, paramL1NpcInstance.getNpcId());
	      	pstm.setInt(2, paramL1NpcInstance.getMapId());
	      	pstm.setInt(3, paramL1NpcInstance.getX());
	      	pstm.setInt(4, paramL1NpcInstance.getY());
