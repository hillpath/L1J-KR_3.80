...
public class BuddyTable {
		...
		try {
			...
			while (charIdRS.next()) {
				try {
					...
					_log.log(Level.SEVERE, "BuddyTable[]Error1", e);
				} finally {
			...
			_log.log(Level.SEVERE, "BuddyTable[]Error2", e);
		} finally {
	...
	public void addBuddy(int charId, int objId, String name) {
		...
		try {
			...
			_log.log(Level.SEVERE, "BuddyTable[]Error3", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
	...
	public void removeBuddy(int charId, String buddyName) {
		...
		try {
			...
			_log.log(Level.SEVERE, "BuddyTable[]Error4", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
