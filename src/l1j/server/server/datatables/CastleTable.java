...
public class CastleTable {
	...
	private void load() {
		...
		try {
			...
			_log.log(Level.SEVERE, "CastleTable[]Error", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	public void updateCastle(L1Castle castle) {
		...
		try {
			  int money;
			   if (castle.getPublicMoney() > 800000000){//2000000000
			    money = 800000000;
			   }else{
			...
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("UPDATE castle SET name=?, war_time=?, tax_rate=?, public_money=?, public_ready_money=?, show_money=?, war_basetime=?, security=? WHERE castle_id=?");
			...
			_log.log(Level.SEVERE, "CastleTable[]Error1", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
