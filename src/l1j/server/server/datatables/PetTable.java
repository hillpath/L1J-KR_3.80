...
public class PetTable {
	...
	public void storeNewPet(L1NpcInstance pet, int objid, int itemobjid) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("INSERT INTO pets SET item_obj_id=?,objid=?,npcid=?,name=?,lvl=?,hp=?,mp=?,exp=?,lawful=?,food=?,foodtime=?");
	...
	public void storePetFoodTime(int id, int food, int foodtime) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("UPDATE pets SET food=?,foodtime=? WHERE objid=?");
			pstm.setInt(1, food);
	...
	public static boolean isNameExists(String nameCaseInsensitive) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			/*
			 * 같은 이름을 찾는다. MySQL는 디폴트로 case insensitive인 싶은 본래 LOWER는 필요없지만,
			 * binary로 변경되었을 경우에 대비해.
			 */
			pstm = con
			...
			if (PetTypeTable.getInstance().isNameDefault(nameLower)) { // 디폴트의
				// 이름이라면
				// 중복 하고
				// 있지
				// 않으면
