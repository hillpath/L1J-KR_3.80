...
public class UBTable {
	...
	public void writeUbScore(int ubId, L1PcInstance pc) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm1 = con
					.prepareStatement("SELECT * FROM ub_rank WHERE ub_id=? AND char_name=?");
			pstm1.setInt(1, ubId);
			...
			if (rs.next()) {
				...
				pstm2 = con
						.prepareStatement("UPDATE ub_rank SET score=? WHERE ub_id=? AND char_name=?");
				...
				pstm2 = con
						.prepareStatement("INSERT INTO ub_rank SET ub_id=?, char_name=?, score=?");
				pstm2.setInt(1, ubId);
