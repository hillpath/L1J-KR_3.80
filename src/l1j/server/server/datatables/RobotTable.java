...
public class RobotTable {
	...
	public void selectRobotTeleportList() { 
		...
		try {
			...
			pstm = con.prepareStatement("select * from robot_teleport_list where isuse = ?");
			pstm.setInt(1, 1);
	...
	public void createRobotTeleportList(L1PcInstance pc) { 
		...
		try {
			int i = 0;
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("insert into robot_teleport_list set x = ?, y = ?, mapid = ?, heading = ?, note = ?, isuse = ?");
